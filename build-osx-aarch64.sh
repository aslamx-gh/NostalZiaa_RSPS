...
APPBASE="build/macos-aarch64/NostalZiaa.app"

build() {
    echo Launcher sha256sum
    shasum -a 256 build/libs/NostalZiaa.jar
    ...
    cp native/build-aarch64/src/NostalZiaa $APPBASE/Contents/MacOS/
    cp build/libs/NostalZiaa.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on NostalZiaa
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/NostalZiaa
    popd

    otool -l $APPBASE/Contents/MacOS/NostalZiaa
}
...
dmg() {
    ...
    mv NostalZiaa\ *.dmg Jirenyte-aarch64.dmg

    # dump for CI
    hdiutil imageinfo NostalZiaa-aarch64.dmg

    if ! hdiutil imageinfo NostalZiaa-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo NostalZiaa-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit NostalZiaa-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple NostalZiaa-aarch64.dmg
    fi
}
